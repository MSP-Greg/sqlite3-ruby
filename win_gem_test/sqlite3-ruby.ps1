...
function Repo-Changes {
  ...
  if (!$is64) { Package-DevKit 'zlib-1.2.8' }
}
...
function Pre-Compile {
  ...
  $sql3 = "$dir_gem/ports".replace('\', '/')
  $env:b_config = "--with-ldflags=-l:libsqlite3.a --with-sqlite3-dir=$sql3 --with-opt-include=$sql3 --with-sqlite3-lib=$sql3/.libs"
}
